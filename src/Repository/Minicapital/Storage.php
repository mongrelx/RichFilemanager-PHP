...
namespace RFM\Repository\Minicapital;
...
class Storage extends BaseStorage implements StorageInterface
{
    ...
    {
        $this->setName(BaseStorage::STORAGE_MINICAPITAL_NAME);
     ...
     */
    public function setMinicapitalClient()
    {
        if (!$this->config('credentials')) {
            throw new \Exception("Minicapital storage credentials isn't set");
        }
    ...
    public function setRoot($path, $makeDir = false)
    {
        ...
        if($makeDir === true && !is_dir($this->storageRoot)) {
            ...
            $this->minicapital->put($this->getDynamicRoot());
        }
    }
     ...
     * @return mixed
     */
    public function getMinicapitalWrapperPath($path)
    {
        $path = $this->cleanPath($this->minicapital->bucket . '/' . $path);

        return 'minicapital://' . $path;
    }
    ...
    {
        $head = $this->minicapital->head($key, true);
    ...
    {
        return $this->minicapital->exist($key);
    }
    ...
    public function createFolder($target, $prototype = null, $options = [])
    {
        ...
        return (bool) $this->minicapital->put($target->getDynamicPath(), '', $options);
    }
    ...
    public function copyItem($source, $target, $remove = false)
    {
        $context = stream_context_create([
            'minicapital' => array_merge($source->getAclParams(), [
                'ServerSideEncryption' => $this->minicapital->encryption,
            ]),
        ]);
        ...
        if ($copied && $remove === true) {
            $this->minicapital->delete($source->getDynamicPath());
        }
    ...
    public function unlinkRecursive($target)
    {
        ...
        if ($target->isDirectory()) {
            $this->minicapital->batchDelete($key);
        } else {
            $this->minicapital->delete($key);
        }
